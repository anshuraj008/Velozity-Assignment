...
- Updated password validation policy in [`packages/shared/src/index.ts`](file:///c:/Projects/Velozity-Assignment/packages/shared/src/index.ts) from min 12 characters to at least 8 characters with at least one uppercase letter, one lowercase letter, one number, and one special symbol. Updated helper hint in [`apps/web/src/components/forms.tsx`](file:///c:/Projects/Velozity-Assignment/apps/web/src/components/forms.tsx).
- Updated demo password across all seed accounts to `FlowDesk@2026` in `apps/server/.env` and `.env`, and updated existing database accounts via scrypt hash migration script.
